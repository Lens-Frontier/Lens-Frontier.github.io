...
如今，Code Agent早已不是光标旁单纯补全程式的辅助工具，Claude code，codex等知名agent似乎已经具备了直接投入生产的能力，向端到端自主软件开发演进已是大势所趋。然而在真实的企业开发环境使用过程中，却发现模型远没有想象中那么能打——一遇到跨文件修改就掉链子，新功能好不容易堆上去，旧逻辑在不知不觉间又崩了，实在让人头疼。
...
* **可靠性低**：所有模型平均通过率普遍低于50%，且P2P回归测试失败率高。结果表明由模型生成大部分代码能跑通部分测试，却无法完整实现功能。本质是模型在跨文件协同方面能力薄弱，且常破坏存量逻辑。
* **效率低**：单任务输入输出Token总量可达数百万，但题目解决率却不高，说明大量算力浪费在无效的代码探索和调试上，离真实开发场景中“高鲁棒性，低成本”的标准仍有一段距离。
* **架构能力欠缺**：脱离现有代码上下文、从零独立设计实现（L2任务），所有模型表现趋于一致的低迷，顶尖模型解决率也仅5%~9%——**"无中生有"式的自主架构设计是当前模型的共同瓶颈。**
...
1. **跨文件依赖解析能力不足**：占比最高的报错是`NameError`。功能横跨多文件时，模型只会修改局部代码，无法维护跨文件引用关系；
