...
lang: "zh"
date: 2026-06-16
summary: "以双重 Gate（行为保持 + 结构静态检查）为核心，用真实开源代码提交做数据、隔离环境测试、用 tree-sitter 做语法树匹配，避开大模型当裁判带来的主观漂移，精准测量大模型的代码重构能力边界。"
authors:
  - name: "韩曙斌｜百度评测与数据组"
    github: "ShubinHan123"
...
今天想讲讲我是如何设计这套评测系统的——**用双重 Gate，把"重构做没做对"变成一个确定性的 0/1 判定，以及在这个过程中踩了哪些坑、做了哪些关键的技术取舍**。
...
核心观察：**重构的定义是"改变结构但不改变行为"——这两件事缺一不可。** 我们的核心设计就是同时验证这两面：用 Docker 跑原项目单测确保行为不变，每个任务用Opus4.6生成 tree-sitter 做静态结构匹配确保结构改对了。
...
单文件重构对强模型基本没有区分度。经实验过程中发现：opus 和 sonnet 在单文件上 Pass@1 分别 77.5% / 75.0%，差距微乎其微；但在跨文件记录上分别 65.9% / 59.1%——**跨文件才能真正拉开差距**。所以最终数据集里 32 条是跨文件（40%），提高跨文件任务的比例。
...
模型改完后，在原项目的 Docker 环境里跑两遍单测：基线一遍、模型完成重构任务后一遍，然后逐项对比。
...
1. **规则生成初版**：Opus4.6结合按重构类型和提交的审计信息（where / what / why）和前后代码差异，产出第一版检Checklist
2. **ground truth 自检过滤**：拿 ground truth跑一遍 Checklist，连ground truth都过不了的检查项直接淘汰
3. **跑一轮模型**：用多个模型各跑一次，形成一张"模型 × 检查项"的通过/失败矩阵
4. **模型复审**：人工结合Opus4.6当分析员，区分"模型确实没做到"和"检查项写得苛刻"
...
早期版本跑过很多额外的 Gate：AST 修改检测、复杂度不退化检测（LLM Judge）、类型检查，以及多维度大模型质量打分。最终全部撤掉。但撤掉的原因各不相同，值得分开说：
...
砍掉这些之后还有一个直接收益：**判分变快并且更准确而且可复现。** 判分链路只剩 BP（容器内跑测试）和 Checklist（纯 tree-sitter，不调大模型），单条记录的验证从"跑多个 Gate 加多次大模型调用"压缩成两步确定性检查。
...
代码重构是Coding Agent 日常使用中最高频的场景之一，这套评测体系的核心价值在于：**用确定性手段（隔离环境测试 + tree-sitter 解析语法树）替代主观判断（大模型当裁判），把重构评测变成一个可复现、可持续、可信赖的 0/1 判定。** 它帮助我们精确定位不同模型在不同语言、不同重构类型上的能力边界，也在持续驱动我们的 agent 策略迭代。
