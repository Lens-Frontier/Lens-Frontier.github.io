...
Use this skill inside the Lens Frontier site repository when helping someone create or edit an article PR.

## Scope Boundary

Use this Skill only for Blog articles and research releases. Structured Benchmark and leaderboard changes follow `docs/ADDING_BENCHMARKS.md`. Do not duplicate a Bench Data Card as a Blog post; create an article only when there is a distinct research narrative or interpretation to publish.
...
3. Use fork-based PRs for normal article and site changes. Confirm the contributor fork remote before pushing, push topic branches to that fork, then open a PR back to the upstream repository.
...
7. Choose the collection:
   - `src/content/papers/<slug>.md` for Lens Frontier papers, preprints, and formal research outputs.
   - `src/content/benchmarks/<slug>.md` for evaluation methods.
...
11. Use `authors` for the contributors shown in the site byline and `paperAuthors` for the formal author order of a paper. For recurring contributors, prefer `id` from `src/data/authors.ts`. If the contributor is not in the registry, either add them there or include at least `name` or `github` inline. Do not add `avatar` by default because the site automatically uses `https://github.com/<github>.png?size=96` when `github` exists. Add `avatar` only when a custom image is needed.
...
14. If the user asks to open the PR and credentials are available, push the branch to the fork and use `gh pr create`. Use a clear title prefix: `post: <article title>` for articles, `docs:`, `site:`, or `chore:` for non-article changes.
...
- A research publication should not be abstract-only. It needs a clear question, method, evidence, contribution, and limits.
...
- For paper posts, use `paperAuthors` for the formal author order and `authors` for the contributors shown in the site byline.
...
After CI succeeds, the PR Preview workflow publishes a commit-scoped preview and posts its URL to the PR. The bot keeps one preview comment updated to the latest commit. Closing the PR triggers cleanup for the whole preview directory.
